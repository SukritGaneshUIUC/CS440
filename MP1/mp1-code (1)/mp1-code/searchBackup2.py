...
def cornerHeuristic(maze, currentNode, mstDict, objectives):
    list.sort(objectives)
    ...
    currentPoint = currentNode.getPoints()[-1]
...
def isObjectiveNode(theNode, objectives):
    ns = theNode.getPoints()
    for o in objectives:
        if (o not in ns):
            return False
    return True

def backtraceNode(maze, theNode):
    ns = theNode.getPoints()
    thePath = []
    for i in range(len(ns) - 1):
        thePath += astarSingle(maze, ns[i], ns[i+1])
    return thePath
...
def astarCornerSingle(maze, start, objectives):
    ...
    frontierQueue = queue.PriorityQueue()
    frontierQueue.put([0 + cornerHeuristic(maze, startNode, mstDict, objectives), 0, startNode])   # contains the list of (h, g, point)
    ...
    while (frontierQueue.qsize() > 0):
        currentNode = frontierQueue.get()
        fs.remove(currentNode[2])
        # print('Queue Size:', frontierQueue.qsize())
        # print('Exploring:', currentPoint)
        exploredNodes.append(currentNode[2])

        # if your current point is an objective, return the path and the objective!
        if (isObjectiveNode(currentNode[2], objectives)):
            # backtrace
            # print('Will Backtrace')
            return backtraceNode(maze, currentNode[2])

        currentNeighbors = getNeighborNodes(currentNode[2], objectives)
        for n in currentNeighbors:
            # add to frontier queue only if you're visiting for first time OR you found shorter path
            newG = currentNode[1] + len(astarSingle(maze, currentNode[2].getPoints()[-1], n.getPoints()[-1]))
            # newG = currentNode[1] + manhattanDistance(currentNode[2].getPoints()[-1], n.getPoints()[-1])
            if (n not in gDict or newG < gDict[n]):  # n not in exploredPoints
                if (n not in fs):
                    fs.add(n)
                frontierQueue.put([newG + cornerHeuristic(maze, n, mstDict, getRemainingObjectives(n, objectives)), newG, n])
                gDict[n.getPoints()[-1]] = newG
...
class Node:
    ...
    def __lt__(self, other):
        return len(self.points) < len(other.getPoints())

    # return TRUE only if both Nodes have the same list of points IN THE SAME ORDER
