...
def viterbi_2(train, test):
    ...
    # Step 0: Find hapax words, as well as tag distribution of hapax words
    # We find how many hapax words appear in each tag
    hapaxSmoothingParameter = 0.5
    ...
    tagFrequencies, tagPairFrequencies, wordTagFrequencies, uniqueWords, uniqueTags = generateFrequencies(train, test, hapaxWords)
...
def generateFrequencies(train, test, hapaxWords):
    ...
    for sentence in train:
        for i in range(len(sentence)):
            ...
            # hapax words are mapped to UNKNOWN
...
def generateHapax(train, hapaxSmoothingParameter):
    ...
    # Step 2: Create hapaxTagFrequencies which maps tags to number of hapax words with that tag
    hapaxTagFrequencies = {}
    for tag in allTags:
    ...
    # Step 3: Generate hapax tag probabilities (we want probabilities, not frequencies)
    hp = generateProbabilities(hapaxTagFrequencies, hapaxSmoothingParameter)
    for tag in hp:
        print(tag, hp[tag])
    return hapaxDict, hp
